...
class llama_model:
    ...
    def _pdf_file_save_class(self, pdf_file, class_name):
        ...
        db.insert_or_update_data(
            pdf_id=pdf_uuid,
            upload_by="123",  # Example teacher ID
    ...
    def _pdf_file_save_selection(
                                self, pdf_file, s_c_ce_type, board_type=None, state_board=None, class_name=None, college_name=None, course_name=None,
                                subject_name=None, stream_type=None
                                ) -> Dict:
        """Saves the PDF file in S3 Bucket if it does not already exist."""
        bucket_name = constants.BUCKETNAME
        if s_c_ce_type == "school":
            if board_type != 'state_board':
                if stream_type is None:
                    file_name = f"{s_c_ce_type}_{board_type}_{class_name}_{pdf_file.name}"
                    pdf_file_path = f"{self.pdf_directory_school}{board_type}/{class_name}/{file_name}"
                else:
                    file_name = f"{s_c_ce_type}_{board_type}_{stream_type}_{class_name}_{pdf_file.name}"
                    pdf_file_path = f"{self.pdf_directory_school}{board_type}/{stream_type}/{class_name}/{file_name}"
            else:
                if stream_type is None:
                    file_name = f"{s_c_ce_type}_{board_type}_{state_board}_{class_name}_{pdf_file.name}"
                    pdf_file_path = f"{self.pdf_directory_school}{board_type}/{state_board}/{class_name}/{file_name}"
                else:
                    file_name = f"{s_c_ce_type}_{board_type}_{state_board}_{stream_type}_{class_name}_{pdf_file.name}"
            ...
            file_name = f"{s_c_ce_type}_{college_name}_{course_name}_{stream_type}_{subject_name}_{pdf_file.name}"
